...
<template>
  <header class="h-[70px] bg-climbe-neutral-card border-b border-climbe-neutral-border flex justify-between items-center px-6 font-avenir box-border w-full sticky top-0 z-50">
    ...
    <div class="flex items-center gap-4">
      ...
      <div class="relative">
        <button 
          class="bg-transparent border border-climbe-neutral-border text-climbe-text-main w-[38px] h-[38px] rounded-full flex items-center justify-center cursor-pointer relative transition-all outline-none hover:border-climbe-primary hover:bg-climbe-primary-light hover:text-climbe-primary-hover"
          :class="{ '!border-climbe-primary !bg-climbe-primary-light !text-climbe-primary-hover': showNotifications }"
          @click="showNotifications = !showNotifications" 
          title="Notificações"
        >
          <span class="text-[1.1rem]">🔔</span>
          <span class="absolute top-[2px] right-[2px] w-2 h-2 bg-climbe-danger border-[1.5px] border-climbe-neutral-card rounded-full"></span>
        </button>
        
        <div v-if="showNotifications" class="absolute top-[46px] right-0 w-[320px] bg-climbe-neutral-card border border-climbe-neutral-border rounded-md shadow-lg overflow-hidden animate-[dropIn_0.25s_cubic-bezier(0.4,0,0.2,1)]">
          <div class="p-3 px-4 bg-climbe-neutral-mute font-heavy text-[0.8rem] uppercase text-climbe-text-muted border-b border-climbe-neutral-border">Notificações Recentes</div>
          <div class="flex flex-col max-h-[280px] overflow-y-auto">
            <div class="p-3 px-4 border-b border-climbe-neutral-border flex gap-2 items-start text-[0.82rem] text-climbe-text-main transition-colors bg-[#5fc2ba0a] hover:bg-climbe-primary-light">
              <span class="w-[6px] h-[6px] bg-climbe-primary rounded-full mt-[6px] shrink-0"></span>
              <div class="flex flex-col gap-0.5">
                <p class="m-0"><strong class="font-heavy">Contrato #1042</strong> aprovado por XP Investimentos.</p>
                <span class="text-[0.7rem] text-climbe-text-muted">Há 15 min</span>
              </div>
            </div>
            <div class="p-3 px-4 border-b border-climbe-neutral-border flex gap-2 items-start text-[0.82rem] text-climbe-text-main transition-colors hover:bg-climbe-primary-light">
              <div class="flex flex-col gap-0.5">
      ...
      <div class="flex items-center gap-3">
        <div class="w-9 h-9 rounded-full bg-gradient-to-br from-climbe-primary to-climbe-primary-hover text-[#121312] flex items-center justify-center font-black text-[0.95rem]">
          {{ userEmail.charAt(0).toUpperCase() }}
        </div>
        <div class="hidden md:flex flex-col">
          <span class="text-[0.85rem] font-heavy text-climbe-text-main leading-[1.2]">Administrador Climbe</span>
          <span class="text-[0.7rem] text-climbe-text-muted">{{ userEmail }}</span>
        </div>
      </div>

      <button class="bg-transparent border border-climbe-neutral-border text-climbe-text-muted w-[34px] h-[34px] rounded-sm flex items-center justify-center cursor-pointer transition-all outline-none hover:border-climbe-danger hover:text-climbe-danger hover:bg-climbe-danger-light" @click="handleLogout" title="Sair do sistema">
