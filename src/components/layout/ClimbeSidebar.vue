...
const menuItems = [
  ...
  { id: 'empresas', label: 'Empresas & Clientes', icon: '🏢' },
...
<template>
  ...
  >
    <div class="h-[70px] flex items-center justify-between px-4 border-b border-white/5 box-border">
      <div
        v-if="!collapsed"
        class="font-black text-[1.2rem] tracking-wider text-white flex items-center gap-1"
      >
        <span>CLIMB</span
        ><span
          class="text-[0.7rem] bg-climbe-primary text-[#121312] px-1 py-0.5 rounded-xs font-heavy"
          >CORP</span
        >
      </div>
        ...
        <li v-for="item in menuItems" :key="item.id">
          ...
          >
            ...
            <span
              ...
              class="absolute left-0 top-1/4 h-1/2 w-[3px] bg-climbe-primary rounded-r-[4px]"
            ></span>
          </button>
        </li>
      </ul>
    ...
    <div class="p-4 border-t border-white/5 box-border">
      <div class="flex items-center gap-3">
        <span class="text-[1.4rem] w-9 h-9 rounded-full bg-white/5 flex items-center justify-center"
          >👨‍💼</span
        >
        <div v-if="!collapsed" class="flex flex-col">
          <span class="text-[0.85rem] font-heavy text-white">Admin Climbe</span>
          <span
            class="text-[0.7rem] text-climbe-primary flex items-center gap-1 before:content-[''] before:inline-block before:w-[6px] before:h-[6px] before:rounded-full before:bg-climbe-primary"
            >Online</span
          >
        </div>
      </div>
