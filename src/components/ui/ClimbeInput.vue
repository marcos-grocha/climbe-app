  ...
  <div class="flex flex-col gap-1 w-full font-avenir" :class="{ 'opacity-60': disabled }">
    <div class="relative flex items-center group" :class="{ 'has-value': hasValue }">
      <input
        ...
        :class="[
          error
            ? 'border-climbe-danger focus:border-climbe-danger focus:ring-climbe-danger-light group-[.has-value]:border-climbe-danger'
            : '',
          $slots.suffix ? 'pr-12' : '',
        ]"
        @input="handleInput"
      ...
      <label
        ...
        :class="[
          error
            ? 'text-climbe-danger peer-focus:text-climbe-danger group-[.has-value]:text-climbe-danger'
            : '',
        ]"
      >
    ...
    <span
      v-if="error"
      class="text-climbe-danger text-[0.8rem] flex items-center gap-1 pl-1 animate-[slideIn_0.25s_ease-out]"
    >
      <svg
        class="shrink-0"
