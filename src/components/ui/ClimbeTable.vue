...
<template>
  <div class="w-full font-avenir flex flex-col gap-4">
    <div
      class="w-full overflow-x-auto border border-climbe-neutral-border rounded-sm bg-climbe-neutral-card"
    >
        ...
        <thead>
          <tr>
            <th
              ...
              :key="index"
              class="px-4 py-3 border-b border-climbe-neutral-border text-climbe-text-muted font-heavy text-[0.8rem] uppercase tracking-wider bg-climbe-neutral-mute select-none"
            >
        ...
        <tbody>
          <template v-if="loading">
            <tr v-for="i in 3" :key="`loading-${i}`">
              <td
                ...
                :key="`col-${j}`"
                class="px-4 py-5 border-b border-climbe-neutral-border"
              >
                <div
                  class="h-[14px] bg-[length:200%_100%] rounded-[4px] animate-[skeletonLoad_1.5s_infinite_linear]"
                  style="
                    background-image: linear-gradient(
                      90deg,
                      var(--climbe-neutral-mute) 25%,
                      var(--climbe-neutral-border) 50%,
                      var(--climbe-neutral-mute) 75%
                    );
                  "
                ></div>
              </td>
          ...
          <template v-else>
            <tr
              ...
              :key="index"
              class="hover:bg-climbe-primary-light transition-colors duration-200 group"
            >
              <td
                v-for="(header, hIndex) in headers"
                :key="hIndex"
                class="px-4 py-4 border-b border-climbe-neutral-border text-climbe-text-main group-last:border-b-0"
              >
    ...
    <div
      ...
      class="flex justify-between items-center text-[0.85rem] px-1"
    >
      <span class="text-climbe-text-muted">
        Página <strong class="text-climbe-text-main font-heavy">{{ currentPage }}</strong> de
        <strong class="text-climbe-text-main font-heavy">{{ totalPages }}</strong>
      </span>
      <div class="flex gap-1">
        ...
        <button
          ...
          :class="
            page === currentPage ? '!bg-climbe-primary !border-climbe-primary !text-[#121312]' : ''
          "
